...
inline bool llc_spandex::is_amo(coh_msg_t coh_msg)
{
    ...
    switch (coh_msg) {
        ...
        case REQ_AMO_MAXU:
        case REQ_AMO_MIN:
        case REQ_AMO_MINU: ret = true; break;
        default: break;
    }
    return ret;
}
...
inline void llc_spandex::send_rsp_out(coh_msg_t coh_msg, line_addr_t addr, line_t line,
...
inline void llc_spandex::send_fwd_out(mix_msg_t coh_msg, line_addr_t addr, cache_id_t req_id,
                                      cache_id_t dest_id, word_mask_t word_mask)
{
    SEND_FWD_OUT;
...
inline void llc_spandex::send_dma_rsp_out(coh_msg_t coh_msg, line_addr_t addr, line_t line,
...
inline void llc_spandex::send_fwd_out_data(mix_msg_t coh_msg, line_addr_t addr, cache_id_t req_id,
                                           cache_id_t dest_id, word_mask_t word_mask, line_t data)
{
    SEND_FWD_OUT;
    llc_fwd_out_t fwd_out;
...
inline bool llc_spandex::send_fwd_with_owner_mask(mix_msg_t coh_msg, line_addr_t addr,
                                                  cache_id_t req_id, word_mask_t word_mask,
                                                  line_t data)
{
    ...
    for (int i = 0; i < WORDS_PER_LINE; i++) {
        if (word_mask & (1 << i)) {
            int owner =
                data.range(CACHE_ID_WIDTH - 1 + i * BITS_PER_WORD, i * BITS_PER_WORD).to_int();
            if (owner != req_id ||
                coh_msg == FWD_RVK_O) { // skip if requestor already owns word, still send if revoke
                bool coal = false;
...
inline bool llc_spandex::send_fwd_with_owner_mask_data(mix_msg_t coh_msg, line_addr_t addr,
                                                       cache_id_t req_id, word_mask_t word_mask,
                                                       line_t data, line_t data_out)
{
    fwd_coal_send_count = 0;
    for (int i = 0; i < WORDS_PER_LINE; i++) {
        if (word_mask & (1 << i)) {
    ...
    for (int i = 0; i < fwd_coal_send_count; i++) {
        send_fwd_out_data(coh_msg, addr, req_id, fwd_coal_temp_dest[i], fwd_coal_word_mask[i],
                          data_out);
        fwd_coal_word_mask[i] = 0;
        wait();
    }
...
// write reqs buf
void llc_spandex::fill_reqs(mix_msg_t msg, cache_id_t req_id, addr_breakdown_llc_t addr_br,
                            llc_tag_t tag_estall, llc_way_t way_hit, llc_unstable_state_t state,
                            hprot_t hprot, word_t word, line_t line, word_mask_t word_mask,
...
bool llc_spandex::reqs_peek_req(addr_breakdown_llc_t br, sc_uint<LLC_REQS_BITS> &reqs_empty_i)
{
    ...
    for (unsigned int i = 0; i < LLC_N_REQS; ++i) {
        ...
        if (reqs[i].state == LLC_I) reqs_empty_i = i;
    ...
    while (true) {
...

        {
            ...
            if (can_get_rsp_in) {
                ...
                if (evict_stall) { req_in = llc_req_stall; }
            ...
            if (is_rsp_to_get) { llc_rsp_in.nb_get(rsp_in); }

            if (is_rst_to_get) { llc_rst_tb.nb_get(rst_in); }

            if (do_get_req) { llc_req_in.nb_get(req_in); }

            if (do_get_dma_req) { llc_dma_req_in.nb_get(dma_req_in); }
        }
                        ...
                        switch (reqs[reqs_hit_i].state) {
                            case LLC_SO: {
                                if (reqs[reqs_hit_i].msg == REQ_O) {
                                    ...
                                    send_rsp_out(RSP_O, rsp_in.addr, 0, reqs[reqs_hit_i].req_id,
                                                 reqs[reqs_hit_i].req_id, 0, 0,
                                                 reqs[reqs_hit_i].word_mask);
                                }
                                else {
                                    HLS_DEFINE_PROTOCOL("send-rsp-804");
                                    send_rsp_out(RSP_Odata, rsp_in.addr, reqs[reqs_hit_i].line,
                                                 reqs[reqs_hit_i].req_id, reqs[reqs_hit_i].req_id,
                                                 0, 0, reqs[reqs_hit_i].word_mask);
                                }
                            ...
                            case LLC_SV: {
                                if (reqs[reqs_hit_i].msg == REQ_WT ||
                                    ...
                                    calc_amo(lines_buf[reqs[reqs_hit_i].way], reqs[reqs_hit_i].line,
                                             req_in.coh_msg, req_in.word_mask);
                                    send_rsp_out(RSP_WTdata, rsp_in.addr, reqs[reqs_hit_i].line,
                                                 reqs[reqs_hit_i].req_id, reqs[reqs_hit_i].req_id,
                                                 0, 0, reqs[reqs_hit_i].word_mask);
                                }
                                else {
                            ...
                            case LLC_SWB: {
                                ...
                                send_mem_req(WRITE, rsp_in.addr, reqs[reqs_hit_i].hprot,
                ...
                case RSP_RVK_O: {
                    for (int i = 0; i < WORDS_PER_LINE; i++) {
                        ...
                        if (owners_buf[reqs[reqs_hit_i].way] & (1 << i) & rsp_in.word_mask) {
                            ...
                            // if (rsp_in.req_id.to_int() ==
                            // lines_buf[reqs[reqs_hit_i].way].range(CACHE_ID_WIDTH - 1 + i *
                            // BITS_PER_WORD, i * BITS_PER_WORD).to_int()) // if owner id == req id
                            // {
                            lines_buf[reqs[reqs_hit_i].way].range((i + 1) * BITS_PER_WORD - 1,
                                                                  i * BITS_PER_WORD) =
                                rsp_in.line.range((i + 1) * BITS_PER_WORD - 1,
                                                  i * BITS_PER_WORD); // write back new data
                            owners_buf[reqs[reqs_hit_i].way] =
                                owners_buf[reqs[reqs_hit_i].way] & (~(1 << i)); // clear owner bit
        ...
        // Process new request
            ...
            set_conflict = reqs_peek_req(evict_addr_br, reqs_hit_i) ||
                reqs_peek_req(addr_br_real, reqs_empty_i) || (reqs_cnt == 0);
            reqs_lookup(addr_br_real, reqs_hit_i);
            ...
            if (evict_stall || set_conflict) // optimize
            {
                ...
                switch (req_in.coh_msg) {
                    case REQ_S: {
                        // if (sharers_buf[reqs[reqs_hit_i].way] & (1 << req_in.req_id)) { // only
                        // allow relaxed rsp_s if private cache has a pending inv
                        //         switch (reqs[reqs_hit_i].state) {
                        //                 case LLC_SI: // to solve reqs vs inv deadlock on LLC_SX
                        //                 due to silent ecivtion
                        //                 {
                        //                         HLS_DEFINE_PROTOCOL("deadlock-solver-1");
                        //                         send_rsp_out(RSP_S, req_in.addr, lines_buf[way],
                        //                         req_in.req_id, req_in.req_id, 0, 0,
                        //                         req_in.word_mask); send_fwd_out(FWD_INV_SPDX,
                        //                         req_in.addr, 0, req_in.req_id, WORD_MASK_ALL);
                        //                         reqs[reqs_hit_i].invack_cnt++;
                        //                         evict_stall = false;
                        //                 }
                        //                 break;
                        //                 case LLC_SO: // to solve reqs vs inv deadlock on LLC_SX
                        //                 due to silent ecivtion
                        //                 {
                ...
                if (evict) {
                    ...
                    switch (states_buf[way]) {
                        ...
                        // because any request conflict in unstable buffer has been blocked by
                        // set_conflict
                        case LLC_V: {
                            if (owners_buf[way] == 0) {
                                if (dirty_bits_buf[way]) {
                                    HLS_DEFINE_PROTOCOL("send_mem_req-2");
                                    send_mem_req(WRITE, addr_evict, hprots_buf[way],
                                                 lines_buf[way]);
                                }
                                states_buf[way] = LLC_I;
                            }
                            else {
                                (void)send_fwd_with_owner_mask(FWD_RVK_O, addr_evict, req_in.req_id,
                                                               owners_buf[way], lines_buf[way]);
                                fill_reqs(FWD_RVK_O, req_in.req_id, evict_addr_br, 0, way, LLC_OWB,
                                          hprots_buf[way], 0, lines_buf[way], owners_buf[way],
                                          reqs_empty_i); // save this request in reqs buffer
                                evict_stall      = true;
                        ...
                        case LLC_S: {
                            ...
                            if (dirty_bits_buf[way])
                                fill_reqs(FWD_INV_SPDX, req_in.req_id, evict_addr_br, 0, way,
                                          LLC_SWB, hprots_buf[way], 0, lines_buf[way],
                                          owners_buf[way],
                                          reqs_empty_i); // save this request in reqs buffer
                            else
                                fill_reqs(FWD_INV_SPDX, req_in.req_id, evict_addr_br, 0, way,
                                          LLC_SI, hprots_buf[way], 0, lines_buf[way],
                                          owners_buf[way],
                                          reqs_empty_i); // save this request in reqs buffer
                ...
                if (evict_stall) { llc_req_stall = req_in; }
                    ...
                    switch (states_buf[way]) {
                        case LLC_I: {
                            ...
                            send_rsp_out(RSP_WTdata, req_in.addr, req_in.line, req_in.req_id,
                                         req_in.req_id, 0, 0,
                                         req_in.word_mask); // send old data out
                        ...
                        case LLC_V: {
                            ...
                            if ((owners_buf[way] & req_in.word_mask) == 0) {
                                calc_amo(lines_buf[way], req_in.line, req_in.coh_msg,
                                         req_in.word_mask);
                                send_rsp_out(RSP_WTdata, req_in.addr, req_in.line, req_in.req_id,
                                             req_in.req_id, 0, 0,
                                             req_in.word_mask); // send old data out
                                ...
                                send_fwd_with_owner_mask(FWD_RVK_O, req_in.addr, req_in.req_id,
                                                         req_in.word_mask, 0);
                                fill_reqs(req_in.coh_msg, req_in.req_id, addr_br_real, 0, way,
                                          LLC_OV, hprots_buf[way], 0, req_in.line, owners_buf[way],
                                          reqs_empty_i); // save this request in reqs buffer
                        ...
                        case LLC_S: {
                            ...
                            fill_reqs(req_in.coh_msg, req_in.req_id, addr_br_real, 0, way, LLC_SV,
                                      req_in.hprot, 0, req_in.line, req_in.word_mask,
                                      reqs_empty_i); // save this request in reqs buffer
                            reqs[reqs_empty_i].invack_cnt = cnt;
                            reqs[reqs_empty_i].is_amo     = true;
                        } break;
                    ...
                    switch (req_in.coh_msg) {
                        ...
                        case REQ_V:
                            ...
                            switch (states_buf[way]) {
                                case LLC_I: {
                                    ...
                                    send_mem_req(READ, req_in.addr, req_in.hprot, 0);
                                    get_mem_rsp(lines_buf[way]);
                                    send_rsp_out(RSP_V, req_in.addr, lines_buf[way], req_in.req_id,
                                                 req_in.req_id, 0, 0, req_in.word_mask);
                                    hprots_buf[way]     = req_in.hprot;
                                    tags_buf[way]       = line_br.tag;
                                    dirty_bits_buf[way] = 0;
                                    states_buf[way]     = LLC_V;
                                ...
                                case LLC_V: {
                                    ...
                                    if (word_owner_mask) {
                                        // if owner exist
                                        send_fwd_with_owner_mask(FWD_REQ_V, req_in.addr,
                                                                 req_in.req_id, word_owner_mask,
                                                                 lines_buf[way]);
                                    }
                                    ...
                                    if (word_no_owner_mask) {
                                        ...
                                        send_rsp_out(RSP_V, req_in.addr, lines_buf[way],
                                                     req_in.req_id, req_in.req_id, 0, 0,
                                                     word_no_owner_mask);
                                    }
                                } break;
                        ...
                        case REQ_S:
                            ...
                            switch (states_buf[way]) {
                                ...
                                case LLC_I: {
                                    ...
                                    send_mem_req(READ, req_in.addr, req_in.hprot, 0);
                                    get_mem_rsp(lines_buf[way]);
                                    send_rsp_out(RSP_S, req_in.addr, lines_buf[way], req_in.req_id,
                                                 req_in.req_id, 0, 0, req_in.word_mask);
                                    hprots_buf[way]     = req_in.hprot;
                                    tags_buf[way]       = line_br.tag;
                                    dirty_bits_buf[way] = 0;
                                ...
                                case LLC_V: {
                                    ...
                                    other_owner        = false;
                                    if (word_owner_mask) {
                                        other_owner = send_fwd_with_owner_mask(
                                            FWD_REQ_S, req_in.addr, req_in.req_id, word_owner_mask,
                                            lines_buf[way]);
                                        if (other_owner) {
                                            fill_reqs(
                                                req_in.coh_msg, req_in.req_id, addr_br_real, 0, way,
                                                LLC_OS, req_in.hprot, 0, lines_buf[way],
                                                req_in.word_mask,
                                                reqs_empty_i); // save this request in reqs buffer
                                        }
                                    }
                                    ...
                                    if (word_no_owner_mask) {
                                        ...
                                        send_rsp_out(RSP_S, req_in.addr, lines_buf[way],
                                                     req_in.req_id, req_in.req_id, 0, 0,
                                                     word_no_owner_mask);
                                        // if other_owner is false, then it means no transient state
                                        // in reqs, and we can change state and shares here
                                        if (!other_owner) {
                                            states_buf[way]  = LLC_S;
                                            sharers_buf[way] = 1 << req_in.req_id;
                                        }
                        ...
                        case REQ_O:
                            ...
                            switch (states_buf[way]) {
                                ...
                                case LLC_I: {
                                    ...
                                    for (int i = 0; i < WORDS_PER_LINE; i++) {
                                        ...
                                        if (req_in.word_mask & (1 << i)) {
                                            lines_buf[way].range(CACHE_ID_WIDTH - 1 +
                                                                     i * BITS_PER_WORD,
                                                                 i * BITS_PER_WORD) = req_in.req_id;
                                        }
                                    }
                                    hprots_buf[way]     = req_in.hprot;
                                    tags_buf[way]       = line_br.tag;
                                    ...
                                    states_buf[way]     = LLC_V;
                                    {
                                        ...
                                        send_rsp_out(RSP_O, req_in.addr, lines_buf[way],
                                                     req_in.req_id, req_in.req_id, 0, 0,
                                                     req_in.word_mask);
                                    }
                                } break;
                                case LLC_V: {
                                    ...
                                    word_no_owner_mask = req_in.word_mask & ~owners_buf[way];
                                    if (word_owner_mask) {
                                        send_fwd_with_owner_mask(FWD_REQ_O, req_in.addr,
                                                                 req_in.req_id, word_owner_mask,
                                                                 lines_buf[way]);
                                    }
                                    if (word_no_owner_mask) {
                                ...
                                case LLC_S: {
                                    ...
                                    for (int i = 0; i < MAX_N_L2; i++) {
                                        ...
                                        if (sharers_buf[way] & (1 << i)) {
                                            if (req_in.req_id == i) {
                                                ...
                                                send_fwd_out(FWD_INV_SPDX, req_in.addr,
                                                             req_in.req_id, i, req_in.word_mask);
                                                cnt++;
                                            }
                                        }
                                        wait();
                                    ...
                                    if (cnt == 0) {
                                        ...
                                        // wait for invack
                                        fill_reqs(req_in.coh_msg, req_in.req_id, addr_br_real, 0,
                                                  way, LLC_SO, req_in.hprot, 0, lines_buf[way],
                                                  req_in.word_mask,
                                                  reqs_empty_i); // save this request in reqs buffer
                                        reqs[reqs_empty_i].invack_cnt = cnt;
                                    }
                            ...
                            switch (states_buf[way]) {
                                ...
                                case LLC_I: {
                                    ...
                                    for (int i = 0; i < WORDS_PER_LINE; i++) {
                                        ...
                                        if (req_in.word_mask & (1 << i)) {
                                            lines_buf[way].range((i + 1) * BITS_PER_WORD - 1,
                                                                 i * BITS_PER_WORD) =
                                                req_in.line.range((i + 1) * BITS_PER_WORD - 1,
                                                                  i * BITS_PER_WORD);
                                        }
                                    }
                                    hprots_buf[way]     = req_in.hprot;
                                        ...
                                        if (req_in.word_mask & (1 << i)) {
                                            lines_buf[way].range((i + 1) * BITS_PER_WORD - 1,
                                                ...
                                                req_in.line.range((i + 1) * BITS_PER_WORD - 1,
                                                                  i * BITS_PER_WORD);
                                            owners_buf[way] =
                                                owners_buf[way] & (~(1 << i)); // clear owner bit
                                        }
                                    }
                                    dirty_bits_buf[way] = 1;
                                } break;
                                case LLC_S: {
                                    ...
                                    int cnt =
                                        send_inv_with_sharer_list(req_in.addr, sharers_buf[way]);
                                    fill_reqs(req_in.coh_msg, req_in.req_id, addr_br_real, 0, way,
                                              LLC_SV, req_in.hprot, 0, lines_buf[way],
                                              req_in.word_mask,
                                              reqs_empty_i); // save this request in reqs buffer
                                    reqs[reqs_empty_i].invack_cnt = cnt;
                                    dirty_bits_buf[way]           = 1;
                        ...
                        case REQ_WTfwd:
                            ...
                            switch (states_buf[way]) {
                                ...
                                case LLC_V: {
                                    ...
                                    word_no_owner_mask = req_in.word_mask & ~owners_buf[way];
                                    if (word_owner_mask) {
                                        send_fwd_with_owner_mask_data(
                                            FWD_WTfwd, req_in.addr, req_in.req_id, word_owner_mask,
                        ...
                        case REQ_Odata:
                            ...
                            // Read with ReqOdata
                            if (req_in.hprot == INSTR) { fcs_prio_buf[way] = 1; }
                            ...
                            switch (states_buf[way]) {
                                ...
                                case LLC_I: {
                                    ...
                                    }
                                    {
                                        ...
                                        send_rsp_out(RSP_Odata, req_in.addr, lines_buf[way],
                                                     req_in.req_id, req_in.req_id, 0, 0,
                                                     req_in.word_mask);
                                    }
                                    owners_buf[way] = req_in.word_mask;
                                ...
                                case LLC_V: {
                                    ...
                                    word_no_owner_mask = req_in.word_mask & ~owners_buf[way];
                                    if (word_owner_mask) {
                                        send_fwd_with_owner_mask(FWD_REQ_Odata, req_in.addr,
                                                                 req_in.req_id, word_owner_mask,
                                                                 lines_buf[way]);
                                    }
                                    if (word_no_owner_mask) {
                                ...
                                case LLC_S: {
                                    ...
                                    if (cnt == 0) {
                                        // only upgrade
                                        HLS_DEFINE_PROTOCOL("send_rsp_1198");
                                        send_rsp_out(RSP_Odata, req_in.addr, temp, req_in.req_id,
                                                     req_in.req_id, 0, 0, req_in.word_mask);
                                    }
                                    else {
                                        // wait for invack
                                        fill_reqs(req_in.coh_msg, req_in.req_id, addr_br_real, 0,
                                                  way, LLC_SO, req_in.hprot, 0, temp,
                                                  req_in.word_mask,
                                                  reqs_empty_i); // save this request in reqs buffer
                                        reqs[reqs_empty_i].invack_cnt = cnt;
                                    }
                        ...
                        case REQ_WB:
                            ...
                            // send response
                            {
                                ...
                                send_rsp_out(RSP_WB_ACK, req_in.addr, 0, req_in.req_id,
                                             req_in.req_id, 0, 0, req_in.word_mask);
                            }
                            ...
                            for (int i = 0; i < WORDS_PER_LINE; i++) {
                                ...
                                if (word_owner_mask & (1 << i)) {
                                    ...
                                    if (req_in.req_id ==
                                        lines_buf[way].range(
                                            CACHE_ID_WIDTH - 1 + i * BITS_PER_WORD,
                                            i * BITS_PER_WORD)) // if owner id == req id
                                    {
                                        lines_buf[way].range((i + 1) * BITS_PER_WORD - 1,
                                                             i * BITS_PER_WORD) =
                                            req_in.line.range(
            ...
            if (reqs_cnt == 0 || reqs_peek_req(evict_addr_br, reqs_empty_i)) {
                ...
                if (!recall_valid && !recall_pending) {
                    ...
                    if (states_buf[way] == LLC_V && owners_buf[way] != 0) {
                        send_fwd_with_owner_mask(FWD_RVK_O, addr_evict, dma_req_in.req_id,
                                                 owners_buf[way], 0);
                        fill_reqs(req_in.coh_msg, req_in.req_id, evict_addr_br, 0, way, LLC_OV,
                                  hprots_buf[way], 0, req_in.line, owners_buf[way], reqs_empty_i);
                        ...
                        dbg_recall_pending.write(recall_pending);
                             ...
                             dma_req_in.coh_msg == REQ_DMA_WRITE_BURST) {
                        int cnt = send_inv_with_sharer_list(addr_evict, sharers_buf[way]);
                        fill_reqs(req_in.coh_msg, req_in.req_id, evict_addr_br, 0, way, LLC_SV,
                                  hprots_buf[way], 0, req_in.line, owners_buf[way], reqs_empty_i);
                        reqs[reqs_empty_i].invack_cnt = cnt;
                        recall_addr                   = addr_evict;
                        recall_pending                = true;
