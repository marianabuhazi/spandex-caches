...
class llc_spandex : public sc_module {
  ...
  public:
    ...
    EXP_MEM_TYPE_STRING(llc_spandex, dirty_bits, LLC_SETS,
                        LLC_WAYS)<sc_uint<2>, LLC_LINES> dirty_bits;
    EXP_MEM_TYPE_STRING(llc_spandex, evict_ways, LLC_SETS,
                        LLC_WAYS)<llc_way_t, LLC_SETS> evict_ways;
    ...
    inline void send_fwd_out_data(mix_msg_t coh_msg, line_addr_t addr, cache_id_t req_id,
                                  cache_id_t dest_id, word_mask_t word_mask, line_t data);
    // returns if any fwd was sent
    inline bool send_fwd_with_owner_mask(mix_msg_t coh_msg, line_addr_t addr, cache_id_t req_id,
                                         word_mask_t word_mask, line_t data);
    inline bool send_fwd_with_owner_mask_data(mix_msg_t coh_msg, line_addr_t addr,
                                              cache_id_t req_id, word_mask_t word_mask, line_t data,
                                              line_t data_out);
    ...
    void fill_reqs(mix_msg_t msg, cache_id_t req_id, addr_breakdown_llc_t addr_br,
