...
inline void write_word_amo(line_t &line, word_t word, word_offset_t w_off, byte_offset_t b_off,
                           hsize_t hsize, amo_t amo)
{
    ...
    switch (hsize) {
        ...
        case HALFWORD: size = 16; break;
        case WORD_32: size = 32; break;
    ...
    bool gt = line.range(line_range_hi, off_bits).to_int() >
        word.range(word_range_hi, b_off_bits).to_int();
    bool ugt = line.range(line_range_hi, off_bits).to_uint() >
        word.range(word_range_hi, b_off_bits).to_uint();
    ...
    switch (amo) {
        case AMO_SWAP:
            ...
            break;
        case AMO_ADD:
            line.range(line_range_hi, off_bits) = line.range(line_range_hi, off_bits).to_int() +
                word.range(word_range_hi, b_off_bits).to_int();
            break;
        case AMO_AND:
            line.range(line_range_hi, off_bits) =
                line.range(line_range_hi, off_bits) & ~(word.range(word_range_hi, b_off_bits));
            break;
        case AMO_OR:
            line.range(line_range_hi, off_bits) =
...
inline void calc_amo(line_t &line, line_t &data, coh_msg_t req, word_mask_t word_mask)
{
    ...
    switch (req) {
        case REQ_AMO_SWAP: old = dataw; break;
        case REQ_AMO_ADD: old += dataw; break;
        case REQ_AMO_AND: old &= dataw; break;
        case REQ_AMO_OR: old |= dataw; break;
        case REQ_AMO_XOR: old ^= dataw; break;
        case REQ_AMO_MAX: old = (old > dataw) ? old : dataw; break;
        case REQ_AMO_MAXU: old = ((unsigned)old > (unsigned)dataw) ? old : dataw; break;
        case REQ_AMO_MIN: old = (old < dataw) ? old : dataw; break;
        case REQ_AMO_MINU: old = ((unsigned)old < (unsigned)dataw) ? old : dataw; break;
        default: break;
    }
    line.range((i + 1) * BITS_PER_WORD - 1, i * BITS_PER_WORD) =
        old; // store calculated new word in the line
}
