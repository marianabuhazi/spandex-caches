...
void l2_spandex::add_wb(bool &success, addr_breakdown_t addr_br, word_t word, l2_way_t way,
                        hprot_t hprot, bool dcs_en, bool use_owner_pred, cache_id_t pred_cid)
{
    // try to add an entry
    ...
    wbs[i].line.range((addr_br.w_off + 1) * BITS_PER_WORD - 1, addr_br.w_off * BITS_PER_WORD) =
        word;
}
...
void l2_spandex::dispatch_wb(bool &success, sc_uint<WB_BITS> wb_i)
{
    ...
    for (unsigned int i = 0; i < N_REQS; ++i) {
        ...
        if (reqs[i].tag == wbs[wb_i].tag && reqs[i].set == wbs[wb_i].set &&
            reqs[i].state != SPX_I) {
...

    {
        ...
        if (!wbs[wb_i].dcs_en) {
            ...
            fill_reqs(0, addr_br, 0, wbs[wb_i].way, 0, SPX_XR, wbs[wb_i].hprot, 0, wbs[wb_i].line,
                      wbs[wb_i].word_mask, reqs_i);
        }
        else if (!wbs[wb_i].use_owner_pred) {
            send_req_out(REQ_WTfwd, wbs[wb_i].hprot, line_addr, wbs[wb_i].line,
                         wbs[wb_i].word_mask);
            fill_reqs(0, addr_br, 0, wbs[wb_i].way, 0, SPX_XRV, wbs[wb_i].hprot, 0, wbs[wb_i].line,
                      wbs[wb_i].word_mask, reqs_i);
        }
        else {
            send_fwd_out(FWD_WTfwd, wbs[wb_i].pred_cid, 1, line_addr, wbs[wb_i].line,
                         wbs[wb_i].word_mask);
            fill_reqs(0, addr_br, 0, wbs[wb_i].way, 0, SPX_XRV, wbs[wb_i].hprot, 0, wbs[wb_i].line,
                      wbs[wb_i].word_mask, reqs_i);
        }
...
void l2_spandex::ctrl()
{
    ...
    while (true) {
...

        {
            ...
            can_get_req_in = ((l2_cpu_req.nb_can_get() && (!drain_in_progress)) || set_conflict) &&
                !evict_stall && !ongoing_fence &&
                (reqs_cnt != 0); // if drain in progress, block all cpu requests
        ...
        if (TEST_new_req && cpu_req.cpu_msg == READ) {
            if (TEST_inverter) { cpu_req.dcs_en = true; }
            TEST_inverter = !TEST_inverter;
        }

        if (cpu_req.dcs_en && cpu_req.cpu_msg == READ) { forced_req_v_dbg.write(1); }
        else {
        ...
        if (do_fence) {
            ...
            switch (rsp_in.coh_msg) {
                case RSP_O: {
                    if (reqs_hit == true) {
                        ...
                        if (fwd_stall && reqs_fwd_stall_i == reqs_hit_i) { fwd_stall_ended = true; }
                    }
                } break;
                case RSP_Odata: {
                    for (int i = 0; i < WORDS_PER_LINE; i++) {
                        ...
                        if (rsp_in.word_mask & (1 << i)) {
                            ...
                            reqs[reqs_hit_i].line.range((i + 1) * BITS_PER_WORD - 1,
                                                        i * BITS_PER_WORD) =
                                rsp_in.line.range((i + 1) * BITS_PER_WORD - 1,
                                                  i * BITS_PER_WORD); // write back new data
                        }
                    }

                    if (reqs[reqs_hit_i].hsize < BYTE_BITS && reqs[reqs_hit_i].state != SPX_AMO) {
                        write_word(reqs[reqs_hit_i].line, cpu_req_conflict.word,
                                   reqs[reqs_hit_i].w_off, reqs[reqs_hit_i].b_off,
                                   reqs[reqs_hit_i].hsize);
                    }
                    ...
                    if (reqs[reqs_hit_i].word_mask == 0) {
                        ...
                        // in case RspOdata is for atomic send rd_rsp immediately and clear
                        // set_conflict
                        if (reqs[reqs_hit_i].state == SPX_AMO) {
                            send_rd_rsp(reqs[reqs_hit_i].line);
                            if (reqs[reqs_hit_i].cpu_msg != READ_ATOMIC) {
                                write_word_amo(reqs[reqs_hit_i].line, cpu_req_conflict.word,
                                               reqs[reqs_hit_i].w_off, reqs[reqs_hit_i].b_off,
                                               reqs[reqs_hit_i].hsize, cpu_req_conflict.amo);
                            }
                            set_conflict = false;
                        }

                        if (reqs[reqs_hit_i].hsize < BYTE_BITS &&
                            reqs[reqs_hit_i].state != SPX_AMO) {
                            set_conflict = false;
                        }

                        if (reqs[reqs_hit_i].state == SPX_IV || reqs[reqs_hit_i].cpu_msg == READ)
                            send_rd_rsp(reqs[reqs_hit_i].line);
                        reqs[reqs_hit_i].state = SPX_I;
                        reqs_cnt++;
                        put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                                 reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, SPX_R, reqs_hit_i);
                    }
                } break;
                ...
                case RSP_WTdata: {
...

                    {
                        ...
                        reqs_cnt++;
                        // put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                        // reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, SPX_I, reqs_hit_i);
                    }
                } break;
                case RSP_V: {
                    ...
                    if (reqs[reqs_hit_i].word_mask == WORD_MASK_ALL) {
                        ...
                        reqs_cnt++;
                        put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                                 reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, current_valid_state,
                                 reqs_hit_i);
                    }
                } break;
                ...
                case RSP_NACK: {
                    switch (reqs[reqs_hit_i].state) {
                        case SPX_XRV: {
                            ...
                            send_req_out(REQ_WT, reqs[reqs_hit_i].hprot,
                                         (reqs[reqs_hit_i].tag, reqs[reqs_hit_i].set),
                                         reqs[reqs_hit_i].line, rsp_in.word_mask);
                        } break;
                        case SPX_IV_DCS: {
                            HLS_DEFINE_PROTOCOL("send reqV miss pred");
                            send_req_out(REQ_V, reqs[reqs_hit_i].hprot,
                                         (reqs[reqs_hit_i].tag, reqs[reqs_hit_i].set), 0,
                                         ~reqs[reqs_hit_i].word_mask);
                        ...
                        case SPX_IV: {
                            reqs[reqs_hit_i].retry++;
                            {
                                ...
                                coh_msg_t retry_msg;
                                if (reqs[reqs_hit_i].retry < MAX_RETRY) { retry_msg = REQ_V; }
                                else {
                                    retry_msg = REQ_Odata;
                                }
                                send_req_out(retry_msg, reqs[reqs_hit_i].hprot,
                                             (reqs[reqs_hit_i].tag, reqs[reqs_hit_i].set), 0,
                                             ~reqs[reqs_hit_i].word_mask);
                ...
                case RSP_S: {
                    if (reqs[reqs_hit_i].state == SPX_IS) {
                        ...
                        if (reqs[reqs_hit_i].word_mask == WORD_MASK_ALL) {
                            ...
                            reqs_cnt++;
                            put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                                     reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, SPX_S,
                                     reqs_hit_i);
                        }
                    }
                        ...
                        if (reqs[reqs_hit_i].word_mask == WORD_MASK_ALL) {
                            ...
                            reqs_cnt++;
                            put_reqs(line_br.set, reqs[reqs_hit_i].way, line_br.tag,
                                     reqs[reqs_hit_i].line, reqs[reqs_hit_i].hprot, SPX_I,
                                     reqs_hit_i);
                        }
                    }
                } break;
                ...
                case RSP_WB_ACK: {
                    if (reqs[reqs_hit_i].state == SPX_RI || reqs[reqs_hit_i].state == SPX_II) {
                        ...
                        // no need to call put_reqs here because there is no state change to either
                        // SRAM or xxx_buf and state to the SRAM was already updated before the
                        // REQ_WB
                        reqs[reqs_hit_i].state = SPX_I;
                        reqs_cnt++;
                    }
            ...
            if (ongoing_atomic && addr_br.line_addr == atomic_line_addr) { ongoing_atomic = false; }
                ...
                switch (fwd_in.coh_msg) {
                    ...
                    case FWD_REQ_O: {
                        ...
                        if (reqs[reqs_fwd_stall_i].state == SPX_RI) {
                            ...
                            send_rsp_out(RSP_O, fwd_in.req_id, true, fwd_in.addr, 0,
                                         fwd_in.word_mask);
                            success = true;
                        }
                        else if (reqs[reqs_fwd_stall_i].state == SPX_XR ||
                                 reqs[reqs_fwd_stall_i].state == SPX_AMO) {
                            word_mask_t rsp_mask = 0;
                            if (tag_hit) {
                                for (int i = 0; i < WORDS_PER_LINE; i++) {
                                    HLS_UNROLL_LOOP(ON, "1");
                                    if ((fwd_in.word_mask & (1 << i)) &&
                                        state_buf[reqs[reqs_fwd_stall_i].way][i] == SPX_R) {
                                        rsp_mask |= 1 << i;
                                ...
                                if (rsp_mask) {
                                    ...
                                    send_rsp_out(RSP_O, fwd_in.req_id, true, fwd_in.addr,
                                                 line_buf[reqs[reqs_fwd_stall_i].way], rsp_mask);
                                    success = true;
                                }
                            }
                        }
                    ...
                    case FWD_REQ_Odata: {
                        ...
                        if (reqs[reqs_fwd_stall_i].state == SPX_RI) {
                            HLS_DEFINE_PROTOCOL("fwd req o stall rsp o");
                            send_rsp_out(RSP_Odata, fwd_in.req_id, true, fwd_in.addr,
                                         reqs[reqs_fwd_stall_i].line, fwd_in.word_mask);
                            success = true;
                        }
                        else if (reqs[reqs_fwd_stall_i].state == SPX_XR ||
                                 reqs[reqs_fwd_stall_i].state == SPX_AMO) {
                    ...
                    case FWD_REQ_V: {
                        ...
                        for (int i = 0; i < WORDS_PER_LINE; i++) {
                            ...
                            if (fwd_in.word_mask & (1 << i)) {
                                ...
                                if ((reqs[reqs_fwd_stall_i].word_mask & (1 << i)) &&
                                    reqs[reqs_fwd_stall_i].state == SPX_RI) {
                                    ack_mask |= 1 << i;
                                }
                                else {
                                    nack_mask |= 1 << i;
                        ...
                        }
                        {
                            HLS_DEFINE_PROTOCOL();
                            if (ack_mask) {
                                send_rsp_out(RSP_V, fwd_in.req_id, true, fwd_in.addr,
                                             reqs[reqs_fwd_stall_i].line, ack_mask);
                            }
                            if (nack_mask) {
                                wait();
                                send_rsp_out(RSP_NACK, fwd_in.req_id, true, fwd_in.addr, 0,
                                             nack_mask);
                            }
                        }
                        success = true;
                    ...
                    case FWD_RVK_O: {
                        if (reqs[reqs_fwd_stall_i].state == SPX_RI) {
                            ...
                            word_mask_t rsp_mask =
                                reqs[reqs_fwd_stall_i].word_mask & fwd_in.word_mask;
                            reqs[reqs_fwd_stall_i].word_mask &= ~rsp_mask;
                            if (rsp_mask)
                                send_rsp_out(RSP_RVK_O, 0, false, addr_br.line_addr,
                                             reqs[reqs_fwd_stall_i].line, rsp_mask);
                            ...
                            if (tag_hit) {
                                ...
                                if (rsp_mask) {
                                    ...
                                    send_rsp_out(RSP_RVK_O, fwd_in.req_id, false, fwd_in.addr,
                                                 line_buf[reqs[reqs_fwd_stall_i].way], rsp_mask);
                                    success = true;
                                }
                            }
                    ...
                    case FWD_REQ_S: {
                        ...
                        if (reqs[reqs_fwd_stall_i].state == SPX_RI) {
                            ...
                            send_rsp_out(RSP_S, fwd_in.req_id, true, fwd_in.addr,
                                         reqs[reqs_fwd_stall_i].line, fwd_in.word_mask);
                            wait();
                            send_rsp_out(RSP_RVK_O, fwd_in.req_id, false, fwd_in.addr,
                                         reqs[reqs_fwd_stall_i].line, fwd_in.word_mask);
                        ...
                        if (tag_hit) {
                            for (int i = 0; i < WORDS_PER_LINE; i++) {
                                ...
                                if ((fwd_in.word_mask & (1 << i)) &&
                                    ...
                                    // if the word in the way is hit in the cache, go to invalid
                                    // state
                    ...
                    case FWD_REQ_O: {
                        ...
                        if (tag_hit) {
                            for (int i = 0; i < WORDS_PER_LINE; i++) {
                                ...
                                if ((fwd_in.word_mask & (1 << i)) &&
                                    state_buf[way_hit][i] ==
                                        SPX_R) { // if reqo and we have this word in registered
                                    // go to invalid state
                                    state_buf[way_hit][i] = SPX_I;
                                    ack_mask |= 1 << i;
                                }
                            ...
                            for (int i = 0; i < WORDS_PER_LINE; i++) {
                                ...
                                if (fwd_in.word_mask & (1 << i)) {
                                    if (state_buf[way_hit][i] == SPX_R) {
                                        line_buf[way_hit].range(BITS_PER_WORD * (i + 1) - 1,
                                                                BITS_PER_WORD * i) =
                                            fwd_in.line.range(BITS_PER_WORD * (i + 1) - 1,
                                                              BITS_PER_WORD * i);
                                        ack_mask |= 1 << i;
                                    }
                                    else {
            ...
            if (set_conflict) {
                ...
                if (ongoing_atomic && cpu_req.hprot == DATA &&
                    addr_br.line_addr != atomic_line_addr) {
                ...
                for (int i = 0; i < WORDS_PER_LINE; i++) {
                    HLS_UNROLL_LOOP(ON);
                    if (state_buf[way_hit][i] == SPX_R) { word_mask_owned |= 1 << i; }
                }
#ifdef L2_DEBUG
                ...
                req_s_needs_evict = tag_hit && !cpu_req.dcs_en && cpu_req.cpu_msg == READ &&
                    word_mask_owned && word_mask_owned != WORD_MASK_ALL;
                if (req_s_needs_evict) { evict_way = way_hit; }

                if (((!tag_hit && !empty_way_found) || req_s_needs_evict) &&
                    !(cpu_req.cpu_msg == WRITE && cpu_req.dcs_en)) {
                    // eviction
                    line_addr_t line_addr_evict =
                        (tag_buf[evict_way] << L2_SET_BITS) | (addr_br.set);
                    ...
                    if (hit_evict) {
                        ...
                        for (int i = 0; i < WORDS_PER_LINE; i++) {
                            ...
                            if (state_buf[evict_way][i] == SPX_R) { word_mask |= 1 << i; }
                        }
                        states.port1[0][base + evict_way] = 0;
                        if (word_mask) {
                            HLS_DEFINE_PROTOCOL("send wb");
                            send_req_out(REQ_WB, hprot_buf[evict_way], line_addr_evict,
                                         line_buf[evict_way], word_mask);
                            fill_reqs(0, evict_addr_br, 0, 0, 0, SPX_RI, 0, 0, line_buf[evict_way],
                                      word_mask, reqs_empty_i);
                        }
                    ...
                    if (cpu_req.amo || cpu_req.cpu_msg == READ_ATOMIC ||
                        cpu_req.cpu_msg == WRITE_ATOMIC) {
                        ...
                        if (cpu_req.amo) {
                            ...
                            if (amo_hit) {
                                ...
                                write_word_amo(line_buf[way_hit], cpu_req.word, addr_br.w_off,
                                               addr_br.b_off, cpu_req.hsize, cpu_req.amo);
                                lines.port1[0][base + way_hit] = line_buf[way_hit];
                            }
                            else {
                                cpu_req_conflict = cpu_req;
                                ...
                                if (tag_hit || empty_way_found) {
                                    ...
                                    fill_reqs(cpu_req.cpu_msg, addr_br, 0, amo_way, cpu_req.hsize,
                                              SPX_AMO, cpu_req.hprot, 0, line_buf[amo_way], 0,
                                              reqs_empty_i);
                                    send_req_out(REQ_Odata, cpu_req.hprot, addr_br.line_addr, 0,
                                                 1 << addr_br.w_off);
                            ...
                            if (amo_hit && lr_line_owned) {
                                ...
                                if (tag_hit || empty_way_found) {
                                    ...
                                    for (int i = 0; i < WORDS_PER_LINE; i++) {
                                        HLS_UNROLL_LOOP(ON);
                                        if (state_buf[amo_way][i] != SPX_R) { amo_wm |= 1 << i; }
                                    }
#ifdef L2_DEBUG
                                    ...
                                    send_req_out(REQ_Odata, cpu_req.hprot, addr_br.line_addr, 0,
                                                 amo_wm);
                                    fill_reqs(READ_ATOMIC, addr_br, addr_br.tag, amo_way,
                                              cpu_req.hsize, SPX_AMO, cpu_req.hprot, 0,
                                              line_buf[amo_way], 0, reqs_empty_i);
                            ...
                            // SC
                            bool sc_success =
                                ongoing_atomic && (addr_br.line_addr == atomic_line_addr);
                        ...
                        if (cpu_req.dcs_en && !tag_hit) {
                            switch (cpu_req.dcs) {
                                case DCS_ReqWTfwd: {
                                    bool success = false;
                                    add_wb(success, addr_br, cpu_req.word, 0, cpu_req.hprot,
                                           cpu_req.dcs_en, cpu_req.use_owner_pred,
                                           cpu_req.pred_cid);
                                    if (!success) {
                            ...
                            write_word(line_buf[way_write], cpu_req.word, addr_br.w_off,
                                       addr_br.b_off, cpu_req.hsize);
                            ...
                            if (cpu_req.dcs_en) {
                                switch (cpu_req.dcs) {
                                    case DCS_ReqWTfwd: {
                                        if ((!word_hit) ||
                                            (state_buf[way_write][addr_br.w_off] !=
                                             SPX_R)) { // if no hit or not in registered
                                            bool success = false;
                                            add_wb(success, addr_br, cpu_req.word, way_write,
                                                   cpu_req.hprot, cpu_req.dcs_en,
                                                   cpu_req.use_owner_pred, cpu_req.pred_cid);
                                            if (!success) {
                                                // if wb refused attempted insertion, raise
                                                // set_conflict
                                                set_conflict = true;
                                ...
                                if ((!word_hit) ||
                                    ...
                                    if (cpu_req.hsize < BYTE_BITS) // partial word write
                                    {
                                        ...
                                        fill_reqs(cpu_req.cpu_msg, addr_br, 0, way_write,
                                                  cpu_req.hsize, SPX_XR, DATA, 0,
                                                  line_buf[way_write], 0, reqs_empty_i);
                                        send_req_out(REQ_Odata, cpu_req.hprot, addr_br.line_addr, 0,
                                                     1 << addr_br.w_off);
                                        reqs[reqs_empty_i].word_mask    = 1 << addr_br.w_off;
                                        reqs_word_mask_in[reqs_empty_i] = 1 << addr_br.w_off;
                                        ...
                                        if (!success) {
                                            ...
                                            state_buf[way_write][addr_br.w_off] =
                                                SPX_R; // directly go to registered
                                        }
                                    }
                                }
                            }
                    ...
                    // assuming line granularity read MESI style
                        ...
                        for (int i = 0; i < WORDS_PER_LINE; i++) {
                            ...
                            if (state_buf[way_hit][i] < current_valid_state ||
                                (cpu_req.dcs == DCS_ReqOdata && state_buf[way_hit][i] != SPX_R)) {
                                word_mask |= 1 << i;
                            }
                        }

                        if (word_mask) { // some words are present but not whole line. send
                                         // fwdreqv/reqv/reqs
                            if (cpu_req.dcs_en && cpu_req.use_owner_pred) {
                                HLS_DEFINE_PROTOCOL("cpu read fwd req v");
                                send_fwd_out(FWD_REQ_V, cpu_req.pred_cid, 1, addr_br.line_addr, 0,
                                             word_mask);
                                fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, way_hit,
                                          cpu_req.hsize, SPX_IV_DCS, cpu_req.hprot, cpu_req.word,
                                          line_buf[way_hit], ~word_mask, reqs_empty_i);
                            }
                            else if (cpu_req.dcs_en) {
                                switch (cpu_req.dcs) {
                                    case DCS_ReqV: {
                                        HLS_DEFINE_PROTOCOL("cpu read req v");
                                        send_req_out(REQ_V, cpu_req.hprot, addr_br.line_addr, 0,
                                                     word_mask);
                                        fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, way_hit,
                                                  cpu_req.hsize, SPX_IV, cpu_req.hprot,
                                                  cpu_req.word, line_buf[way_hit], ~word_mask,
                                                  reqs_empty_i);
                                    } break;
                                    case DCS_ReqOdata: {
                                        HLS_DEFINE_PROTOCOL("cpu read req o data");
                                        send_req_out(REQ_Odata, INSTR, addr_br.line_addr, 0,
                                                     word_mask);
                                        fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, way_hit,
                                                  cpu_req.hsize, SPX_XR, INSTR, cpu_req.word,
                                                  line_buf[way_hit], 0, reqs_empty_i);
                                        reqs[reqs_empty_i].word_mask    = 1 << addr_br.w_off;
                                        reqs_word_mask_in[reqs_empty_i] = 1 << addr_br.w_off;
                                    } break;
                                    default: break;
                                ...
                                send_req_out(REQ_S, cpu_req.hprot, addr_br.line_addr, 0,
                                             WORD_MASK_ALL);
                                fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, way_hit,
                                          cpu_req.hsize, SPX_IS, cpu_req.hprot, cpu_req.word,
                                          line_buf[way_hit], 0, reqs_empty_i);
                            }
                        }
                        else {
                        ...
                        if (cpu_req.dcs_en && cpu_req.use_owner_pred) {
                            ...
                            send_fwd_out(FWD_REQ_V, cpu_req.pred_cid, 1, addr_br.line_addr, 0,
                                         WORD_MASK_ALL);
                            fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, empty_way,
                                      cpu_req.hsize, SPX_IV_DCS, cpu_req.hprot, cpu_req.word,
                                      line_buf[empty_way], 0, reqs_empty_i);
                        }
                        else if (cpu_req.dcs_en) {
                            switch (cpu_req.dcs) {
                                case DCS_ReqV: {
                                    ...
                                    send_req_out(REQ_V, cpu_req.hprot, addr_br.line_addr, 0,
                                                 WORD_MASK_ALL);
                                    fill_reqs(cpu_req.cpu_msg, addr_br, addr_br.tag, empty_way,
                                              cpu_req.hsize, SPX_IV, cpu_req.hprot, cpu_req.word,
                                              line_buf[empty_way], 0, reqs_empty_i);
                                } break;
                                case DCS_ReqOdata: {
...
inline void l2_spandex::send_req_out(coh_msg_t coh_msg, hprot_t hprot, line_addr_t line_addr,
...
inline void l2_spandex::send_rsp_out(coh_msg_t coh_msg, cache_id_t req_id, bool to_req,
                                     line_addr_t line_addr, line_t line, word_mask_t word_mask)
{
    // SEND_RSP_OUT;
...
inline void l2_spandex::send_fwd_out(coh_msg_t coh_msg, cache_id_t dst_id, bool to_dst,
                                     line_addr_t line_addr, line_t line, word_mask_t word_mask)
{
    // SEND_FWD_OUT;
...
void l2_spandex::fill_reqs(cpu_msg_t cpu_msg, addr_breakdown_t addr_br, l2_tag_t tag_estall,
                           l2_way_t way_hit, hsize_t hsize, unstable_state_t state, hprot_t hprot,
                           word_t word, line_t line, word_mask_t word_mask,
                           sc_uint<REQS_BITS> reqs_i)
{
    FILL_REQS;
...
void l2_spandex::put_reqs(l2_set_t set, l2_way_t way, l2_tag_t tag, line_t line, hprot_t hprot,
                          spx_state_t state, sc_uint<REQS_BITS> reqs_i)
{
    PUT_REQS;
    ...
    for (int i = 0; i < WORDS_PER_LINE; i++) {
        ...
        if (reqs_word_mask_in[reqs_i] & 1 << i) state_buf[way][i] = state;
    }
...
void l2_spandex::tag_lookup(addr_breakdown_t addr_br, bool &tag_hit, l2_way_t &way_hit,
                            bool &empty_way_found, l2_way_t &empty_way, bool &word_hit)
{
    TAG_LOOKUP;
    ...
    for (int i = L2_WAYS - 1; i >= 0; --i) {
        ...
        for (int j = 0; j < WORDS_PER_LINE; j++) {
            ...
            if (state_buf[i][j] >= current_valid_state) { line_present = true; }
        }
        ...
        if (tag_buf[i] == addr_br.tag && line_present &&
            state_buf[i][addr_br.w_off] >= current_valid_state) {
            word_hit = true;
        }
...
void l2_spandex::reqs_lookup(line_breakdown_t<l2_tag_t, l2_set_t> line_br,
...
void l2_spandex::reqs_peek_req(l2_set_t set, sc_uint<REQS_BITS> &reqs_i)
{
    ...
    for (unsigned int i = 0; i < N_REQS; ++i) {
        ...
        if (reqs[i].state == SPX_I) reqs_i = i;

        if (reqs[i].set == set && reqs[i].state != SPX_I) { set_conflict = true; }
    }
...
void l2_spandex::reqs_peek_fwd(addr_breakdown_t addr_br)
{
    ...
    for (unsigned int i = 0; i < N_REQS; ++i) {
        ...
        if (reqs[i].tag == addr_br.tag && reqs[i].set == addr_br.set && reqs[i].state != SPX_I) {
            ...
            if (ongoing_atomic && reqs[i].cpu_msg == READ_ATOMIC) { ongoing_atomic = false; }
        }
    }
}
...
{
    if (current_valid_state != SPX_MAX_V) { current_valid_state++; }
    else {
        for (int i = 0; i < L2_LINES; i++) {
            ...
            for (int j = 0; j < WORDS_PER_LINE; j++) {
                ...
                if (state_tmp != SPX_R && state_tmp != SPX_S) { state_tmp = SPX_I; }
...
void l2_spandex::flush()
{
    ...
    for (unsigned int i = 0; i < WORDS_PER_LINE; i++) {
        ...
        if (word_state == SPX_R) { flush_owned |= 1 << i; }
    }
    ...
    if (flush_owned == 0) {
        ...
        for (unsigned int i = 0; i < N_REQS; ++i) {
            ...
            if (reqs[i].state == SPX_I) { reqs_i = i; }
            if (reqs[i].tag == line_tag && reqs[i].set == flush_line >> L2_WAY_BITS &&
                reqs[i].state != SPX_I) {
                // found conflict, cannot dispatch WB
                success = false;
                return;
